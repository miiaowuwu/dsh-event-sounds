// dsh-sound-lab 多 profile 自动检测配置脚本（运行：node tools/install.mjs）
//
...
//   --npm       改用 npm registry 安装（spec = dsh-sound-lab，并带
...
const NPM_MODE = args.includes("--npm"); // npm 源安装（dsh-sound-lab），不走 link
...
console.log("dsh-sound-lab 多 profile 自动配置（" + (DRY ? "dry-run，仅检测" : "实际配置") + "）");
