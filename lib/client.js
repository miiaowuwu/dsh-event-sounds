	...
	factory: (require) => {
		...
		function AiTtsModal(props) {
				...
				return React.createElement("div", { className: "sc-tts-tut-step", key: i },
					...
					React.createElement("div", { className: "sc-tts-tut-text" },
						...
						!isStr ? React.createElement("img", {
							...
							alt: "教程示意图" + (i + 1),
							title: "点击放大查看",
							onClick: () => setZoomImg(window.location.origin + SOUNDS_BASE + "/tts/tutorial/" + s.img),
						}) : null,
					),
				);
			});
			...
			return React.createElement(React.Fragment, null,
				React.createElement("div", { className: "sc-upload-popup sc-tts-popup" + (showTutorial ? " sc-tts-expanded" : ""), style: { left: pos.x, top: pos.y } },
				React.createElement("div", { className: "sc-upload-head", onPointerDown: onHeadDown },
				...
				// 教程图片放大层（点击图片打开，点击遮罩/图片关闭）
				zoomImg ? React.createElement("div", { className: "sc-tts-zoom-overlay", onClick: () => setZoomImg(null) },
					React.createElement("img", { className: "sc-tts-zoom-img", src: zoomImg, alt: "教程示意图（放大）" }),
				) : null,
			);
		}
		...
		const CSS_TEXT = [
			...
			// 教程配图按视口高度缩放（上限 20vh），显著降低教程整体高度，避免撑出滚动条
			".sc-tts-tut-img { display: block; max-width: 100%; max-height: 20vh; width: auto; height: auto; margin-top: 4px; border-radius: 8px; border: 1px solid rgba(190,153,82,0.4); cursor: zoom-in; }",
			".sc-tts-zoom-overlay { position: fixed; left: 0; top: 0; right: 0; bottom: 0; z-index: 2147483001; background: rgba(0,0,0,0.78); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; }",
			".sc-tts-zoom-img { max-width: 100%; max-height: 100%; border-radius: 10px; box-shadow: 0 10px 48px rgba(0,0,0,0.65); }",
