/**
 * sounds-control 浏览器端（Client 半）v3 —— 深海女仆工坊（鲸鱼娘）风格。
 *
...
window.__ModuleLoader__.load({
	id: "dsh-client-ui-sounds-control",
	factory: (require) => {
		var module = { exports: {} };
		...
		// 判断最后一条 assistant 消息是否是被「停止」中断的（interrupted 标记）
		function wasInterrupted(session) {
			...
			for (let i = nodes.length - 1; i >= 0; i--) {
				...
				// 跳过回合尾等非内容节点，继续往前找最近的 assistant 节点
				if (n.kind === "user" || n.kind === "tool-call") continue;
			}
			return false;
		}
		...
		function Watcher(props) {
			...
			React.useEffect(() => {
				...
				if (newKind) {
					...
					// running 结束：被停止则走「停止」触发，否则走「回合结束」
					play(wasInterrupted(session) ? "stop" : "end");
				}
			}, [session]);
			return null;
		}
			...
			return React.createElement("div", { className: "sc-popup", "data-sc-style": props.style || "maid", style: { left: px, top: py } },
				...
				React.createElement("div", { className: "sc-popup-body" },
					...
					React.createElement("div", { className: "sc-section" },
						...
						triggerRows,
						React.createElement("div", { className: "sc-hint" }, "勾选启用，下拉选择该触发条件要播放的音效；留空则该触发条件不播放。"),
					),
					// 外观
					React.createElement("div", { className: "sc-section" },
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				el.textContent = "sounds-control 已加载";
				el.setAttribute("data-plugin", "sounds-control-boot");
			...
			ctx.effect(() => {
				...
				tag.setAttribute("data-plugin", "dsh-client-ui-sounds-control");
			...
			try {
				slots.inject("shell.overlay", () => slots.register(
					{ name: "shell.overlay", id: "sounds-control-root", order: 60 },
			...
			try {
				slots.inject("conversation.input.dock", () => slots.register(
					{ name: "conversation.input.dock", id: "sounds-control-watcher", order: 80 },
