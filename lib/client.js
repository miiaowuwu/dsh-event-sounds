...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ==================== 悬浮按钮（可拖动 / 靠边缩成半球）====================
		// 标题栏（工具/帮助/收起/缩放/关闭所在行）下边缘 y（视口坐标）。
		// maid-atelier 皮肤把该行下边缘测量进了 --maid-titlebar-height（其 top-trim 壁纸
		// 也用它避开标题栏行），优先读取；其次直接量 [class*='titlebar'] 元素；
		// 都没有（web 版无标题栏）返回 0 → 保持贴屏幕顶的原行为。
		function titleBarBottom() {
			try {
				const v = getComputedStyle(document.body).getPropertyValue("--maid-titlebar-height");
				const n = parseFloat(v);
				if (isFinite(n) && n > 0) return Math.round(n);
			} catch (err) { /* ignore */ }
			try {
				const el = document.querySelector("[class*='titlebar']");
				if (el && typeof el.getBoundingClientRect === "function") {
					const r = el.getBoundingClientRect();
					if (r.bottom > 0) return Math.round(r.bottom);
				}
			} catch (err) { /* ignore */ }
			return 0;
		}
		...
		function buttonStyle(geo) {
			...
			if (d === "right") {
				...
				// 顶部吸附：小半球停靠在标题栏下边缘（不遮挡 工具/帮助/收起/缩放/关闭 按钮行）
				s.top = titleBarBottom(); s.left = Math.max(0, cx - DOCK_H / 2); s.width = DOCK_H; s.height = DOCK_W;
		...
		function FloatingButton(props) {
			...
			const onPointerDown = (e) => {
				...
				const onUp = (ev) => {
					...
					else if (cy <= titleBarBottom() + 34) docked = "top";
		...
		function HirariRoot() {
			...
			// 悬浮球渲染在弹窗之后（DOM 靠后）：z-index 同为最大值 2147483647 时，悬浮球压在弹窗之上
			return React.createElement(React.Fragment, null,
		...
		const CSS_TEXT = [
			// 宿主 shell.overlay 容器自身 z-index:20，会把内部悬浮球/弹窗（2147483647）困在该层，
			// 宿主更高/同级后渲染的浮层（上边框壁纸、文件/变更面板等）会盖住它们；
			// 提升容器到最高层 → 悬浮球与弹窗始终在宿主所有浮层之上。
			"[data-shell-overlay] { z-index: 2147483000 !important; }",
			// 皮肤上边框壁纸（top-trim）挂在 body 末端、z-index:20，与 shell.overlay 容器（z-index:20）
			// 同级且 DOM 靠后，会压过容器内的悬浮球/弹窗（它们的 2147483647 只在容器内有效）；
			// 降到与侧边栏内容同级（z-index:0），悬浮球/弹窗才能压在壁纸之上。
			"[data-skin-chrome='top-trim'] { z-index: 0 !important; }",
			// 上边框区域按钮（模式切换/会话等 workspace header）z-index:21 高于弹窗容器（20），
			// 同样会压住容器内元素；降到 19（低于容器 20，仍高于壁纸 0，视觉不变）。
			"body[data-dsh-maid-atelier] header:has([role='tablist']) { z-index: 19 !important; }",
			// 悬浮按钮：侧栏珠宝按钮同款 —— 金边 + 海军蓝渐变 + 内嵌金环
			".sc-btn { position: fixed; z-index: 2147483647; display: flex; align-items: center; justify-content: center; cursor: grab; user-select: none; touch-action: none; padding: 0; border: 1px solid rgba(225,191,124,0.76); background: linear-gradient(145deg, rgba(77,103,169,0.55), rgba(7,18,52,0.85)); color: #ebd29e; box-shadow: inset 0 0 0 2px rgba(5,15,45,0.76), 0 3px 9px rgba(1,7,24,0.28); }",
			...
			".sc-tts-zoom-overlay { position: fixed; left: 0; top: 0; right: 0; bottom: 0; z-index: 2147483001; background: rgba(0,0,0,0.78); display: flex; align-items: center; justify-content: center; padding: 24px; cursor: zoom-out; overflow: hidden; touch-action: none; }",
