...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function AiTtsModal(props) {
			...
			return React.createElement("div", { className: "sc-upload-popup sc-tts-popup" + (showTutorial ? " sc-tts-expanded" : ""), style: { left: pos.x, top: pos.y } },
				React.createElement("div", { className: "sc-upload-head", onPointerDown: onHeadDown },
		...
		const CSS_TEXT = [
			...
			".sc-tts-tut-img { display: block; max-width: 100%; max-height: 20vh; width: auto; height: auto; margin-top: 4px; border-radius: 8px; border: 1px solid rgba(190,153,82,0.4); }",
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				tag.setAttribute("data-plugin", "dsh-event-sounds");
